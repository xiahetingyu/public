...
<html>
  ...
  <body style="background-color: #f0f9fd;">
  	...
  	<form action="${ pageContext.request.contextPath }/clazz/add" method="post">
		所属一级类别:		
		<select name="clazz.p" class="el-select__inner">
			<c:forEach items="${ clazzs }" var="clazz">
				<option value="${ clazz.id }">${ clazz.name }</option>
			</c:forEach>
		</select>
  		<br/><br/>
  		二级类别名:<input class="el-input__inner" type="text" name="clazz.name" id="sname"/><br/><br/>
  		<input class="button btn-p" type="submit" value="提交"/>&emsp;
  		<input name="clazz.level" value="2" type="hidden"/>

...
public class Implement_89IP implements Collector {
	// 请求头
	private static String[] property = Tools.merge(Configuration.BROWSER, new String[] { "Host", "www.89ip.cn" });
	...
	public List<ProxyE> pattern(String content) {
		...
		while (matcher.find()) {
			...
			if (strings != null && strings[0] != "") {
				ProxyE e = new ProxyE();
				e.setIp(strings[0].replaceAll("<td>", ""));
